...
async function initPhongShader() {
    const vertexShaderSource = await readShaderFile('shVertPho.glsl');
    const fragmentShaderSource = await readShaderFile('shFragPho.glsl');
    phongShader = new Shader(gl, vertexShaderSource, fragmentShaderSource);
}

async function initGouraudShader() {
    const vertexShaderSource = await readShaderFile('shVertGou.glsl');
    const fragmentShaderSource = await readShaderFile('shFragGou.glsl');
